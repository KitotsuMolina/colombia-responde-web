...
import type { ApiIncident, ApiIncidentDetail, ApiLocation, ApiMissingPerson, ApiSafetyCheckIn, AreaPoint, Incident, MissingPerson, ReportKind } from './types'
...
const incidentIcon=(item:Incident,offset={x:0,y:0})=>{const meta=kindMeta[item.kind];return divIcon({className:'leaflet-report-icon',html:`<span class="leaflet-marker ${meta.tone}"><i>${markerGlyph[item.kind]}</i></span>`,iconSize:[38,46],iconAnchor:[19-offset.x,44-offset.y],popupAnchor:[offset.x,offset.y-42]})}
function IncidentMarkers({items,onSelect}:{items:Incident[];onSelect:(id:string)=>void}){
  const areas=items.filter(item=>item.area&&item.area.length>=3).map(item=>{const meta=kindMeta[item.kind],color=meta.tone==='red'?'#d94b3f':meta.tone==='green'?'#26845c':'#e6813e';return <Polygon key={`area-${item.sourceId}`} positions={item.area!.map(point=>[point.latitude,point.longitude])} pathOptions={{color,fillColor:color,fillOpacity:.24,weight:3}} eventHandlers={{click:()=>onSelect(item.sourceId)}}><Popup><b>Área aproximada · {meta.label}</b><br/>{item.title}<br/><small>Toca para ver detalles y fotos</small></Popup></Polygon>})
  return <>{areas}{groupNearbyIncidents(items).flatMap(group=>group.items.map((item,index)=>{const meta=kindMeta[item.kind],angle=(Math.PI*2*index/group.items.length)-Math.PI/2,radius=group.items.length>1?Math.min(48,26+group.items.length*3):0,offset={x:Math.cos(angle)*radius,y:Math.sin(angle)*radius};return <Marker eventHandlers={{click:()=>onSelect(item.sourceId)}} key={item.sourceId} position={[group.latitude,group.longitude]} icon={incidentIcon(item,offset)} zIndexOffset={index}><Popup><b>{item.area?'Área aproximada · ':''}{meta.label}</b><br/>{item.title}<br/><small>{group.items.length>1?`${index+1} de ${group.items.length} reportes en este lugar · `:''}Toca para ver detalles y fotos</small></Popup></Marker>}))}</>
}
...
function IncidentDetailPage({id,close}:{id:string;close:()=>void}){const[data,setData]=useState<ApiIncidentDetail>(),[error,setError]=useState('');useEffect(()=>{api.incident(id).then(setData).catch(reason=>setError(reason instanceof Error?reason.message:'No fue posible cargar el reporte'))},[id]);if(error)return <main className="form-page"><button className="back" onClick={close}><ArrowLeft/> Volver</button><p className="form-error">{error}</p></main>;if(!data)return <main className="form-page"><div className="loading"><LoaderCircle/> Cargando detalle…</div></main>;const meta=kindMeta[data.kind];return <main className="form-page incident-detail"><button className="back" onClick={close}><ArrowLeft/> Volver al mapa</button><span className="section-kicker">{meta.label.toUpperCase()}</span><h1>{data.title}</h1><p><MapPin size={15}/>{data.location.locality?`${data.location.locality} · `:''}{data.location.municipalityName} · {data.location.departmentName}</p><div className="verification"><ShieldCheck size={14}/> {data.verificationStatus==='unverified'?'Reporte ciudadano sin verificar':'Evidencia adjunta'}</div><p className="detail-description">{data.description}</p>{data.peopleAtRisk!==undefined&&<p><b>Personas en riesgo:</b> {data.peopleAtRisk}</p>}<h2>Fotografías</h2>{data.evidence.length?<div className="evidence-gallery">{data.evidence.map(photo=><a href={photo.url} target="_blank" rel="noreferrer" key={photo.id}><img src={photo.url} alt={`Evidencia de ${data.title}`} loading="lazy"/></a>)}</div>:<div className="no-evidence"><Camera/> Este reporte no tiene fotografías.</div>}<div className="privacy"><ShieldCheck/><p>Las fotografías son evidencia ciudadana y no implican verificación oficial.</p></div></main>}
...
export default function App(){return location.pathname.startsWith('/admin')?<AdminPage/>:<PublicApp/>}
