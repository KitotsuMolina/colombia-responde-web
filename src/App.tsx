...
import type { ApiIncident, ApiIncidentDetail, ApiLocation, ApiMissingPerson, ApiSafetyCheckIn, Incident, MissingPerson, ReportKind } from './types'
...
function MapPanel({ items, onLocate, onSelect=openIncidentDetail, userCoordinates }: { items: Incident[]; onLocate: () => void;onSelect?:(id:string)=>void; userCoordinates?: Coordinates }) {
  return <section className="map-card"><div className="map-head"><div><span className="section-kicker">SITUACIÓN ACTUAL</span><h2>Mapa ciudadano</h2></div><span className="region"><MapPin size={15}/> Colombia <ChevronDown size={14}/></span></div>
    <div className="map-canvas leaflet-map"><MapContainer center={[4.5709,-74.2973]} zoom={5} minZoom={4} scrollWheelZoom={false}>
      ...
      {items.map(item => { const meta=kindMeta[item.kind]; return <Marker eventHandlers={{click:()=>onSelect(item.sourceId)}} key={item.id} position={[item.latitude,item.longitude]} icon={divIcon({className:'leaflet-report-icon',html:`<span class="leaflet-marker ${meta.tone}"><i>${markerGlyph[item.kind]}</i></span>`,iconSize:[38,46],iconAnchor:[19,44],popupAnchor:[0,-42]})}><Popup><b>{meta.label}</b><br/>{item.title}<br/><small>Toca para ver detalles y fotos</small></Popup></Marker> })}
